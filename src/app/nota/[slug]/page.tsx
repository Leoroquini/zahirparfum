...
export default async function NotaPage({ params }: Props) {
  ...
  return (
    <article className="relative">
      {/* Hero da nota */}
      <section className="relative flex min-h-[60vh] items-end overflow-hidden border-b border-cream/5 px-6 pb-14 pt-32 md:min-h-[70vh] md:px-12 md:pb-20 md:pt-40">
        <Image
          src={nota.foto}
          alt={nota.nome}
          fill
        ...
        <div className="relative mx-auto w-full max-w-[1440px]">
          ...
          <div className="mt-8 flex flex-col gap-6">
            ...
            <h1 className="max-w-4xl font-display text-5xl font-light leading-[1] tracking-tight text-cream md:text-7xl lg:text-8xl">
              {nota.nome}
            </h1>
            <p className="max-w-3xl font-display text-xl italic leading-relaxed text-amber/85 md:text-2xl">
              {nota.subtitulo}
            </p>
            <div className="mt-4 flex flex-wrap gap-x-8 gap-y-3 text-sm text-cream/70">
              <div className="flex flex-col gap-1">
                <span className="text-[9px] font-sans uppercase tracking-[0.4em] text-cream/40">
                  Origem
                </span>
        ...
        <div className="mx-auto flex max-w-3xl flex-col gap-12">
          ...
          {nota.conteudo.curiosidade && (
            <div className="rounded-sm border border-amber/25 bg-amber/5 p-6 md:p-8">
              ...
              <p className="mt-3 font-display text-base font-light italic leading-relaxed text-cream/85 md:text-lg">
                {nota.conteudo.curiosidade}
              </p>
            </div>
          )}
      ...
      <section className="border-t border-cream/5 bg-ink-soft px-6 py-20 md:px-12 md:py-28">
        <div className="mx-auto max-w-[1440px]">
          ...
          {perfumes.length === 0 ? (
            <p className="mt-14 max-w-md text-sm italic text-cream/50">
      ...
      {/* Outras notas */}
      <section className="border-t border-cream/5 bg-ink px-6 py-16 md:px-12 md:py-20">
        <div className="mx-auto max-w-[1440px]">
          <span className="text-[10px] font-sans uppercase tracking-[0.45em] text-amber">
            <span className="mr-3 inline-block h-px w-8 align-middle bg-amber" />
          ...
          <ul className="mt-8 grid gap-4 md:grid-cols-3">
            {NOTAS.filter((n) => n.id !== nota.id).map((outra) => (
              <li key={outra.id}>
                <Link
                  href={`/nota/${outra.id}`}
                  className="group flex items-center gap-4 rounded-sm border border-cream/10 bg-ink-soft p-5 transition-all hover:border-amber/40 hover:bg-ink-muted"
                >
...
function BlocoEditorial({
  ...
  return (
    <div className="flex flex-col gap-4 border-l-2 border-amber/40 pl-6 md:pl-8">
      ...
      <p className="font-display text-base font-light leading-[1.6] text-cream/85 md:text-lg">
        {conteudo}
      </p>
    </div>
  );
