            ...
            {NOTAS.map((nota) => {
              ...
              return (
                <Link
                  key={nota.id}
                  href={`/nota/${nota.id}`}
                  className="group relative flex aspect-[5/4] flex-col justify-between overflow-hidden rounded-sm border border-cream/10 p-6 transition-all duration-700 hover:border-amber/50 hover:shadow-[0_0_60px_rgba(200,155,60,0.15)] md:p-8"
                >
                  <Image
                    src={nota.foto}
                    alt={nota.nome}
                  ...
                  <div className="relative flex flex-col gap-2">
                    <h2 className="font-display text-4xl font-light leading-[1.05] text-cream transition-colors duration-500 group-hover:text-amber-bright md:text-5xl">
                      {nota.nome}
                    </h2>
